...
complete -c moon -n "__fish_moon_using_subcommand tool; and not __fish_seen_subcommand_from format-and-diff embed write-tcc-rsp-file build-binary-dep help" -l manifest-path -d 'Path to `moon.mod.json` to use as the project manifest (does not change the working directory)' -r -F
complete -c moon -n "__fish_moon_using_subcommand tool; and not __fish_seen_subcommand_from format-and-diff embed write-tcc-rsp-file build-binary-dep help" -l target-dir -d 'The target directory. Defaults to `<project-root>/_build`' -r -F
