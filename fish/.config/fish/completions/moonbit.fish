...
complete -c moon -n "__fish_moon_using_subcommand help; and not __fish_seen_subcommand_from new bundle build check run test generate-test-driver clean fmt doc info bench add remove install tree login register publish package update coverage generate-build-matrix query upgrade shell-completion version tool help" -f -a "generate-test-driver" -d 'Generate tests for a provided package. This is a thin wrapper around `moonc gen-test-info`, which does the actual parsing and generation'
